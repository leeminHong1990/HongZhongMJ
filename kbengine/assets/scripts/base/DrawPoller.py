...
class DrawPoller:
	...
	def processData(self, sock, datas):
		...
		try:
			...
			INFO_MSG("prcessData decode: %s" % datasStr)
			(datasStr, tile) = datasStr.split("&tile=")
			tile = (re.findall(r"\d+\.?\d*", tile))[0]
			tile = int(tile)
			(datasStr, roomID) = datasStr.split("roomID=")
			roomID = int(roomID)
			INFO_MSG("prcessData: %s, %s" % (roomID, tile))
			KBEngine.globalData["GameWorld"].swapTileToTop(roomID, tile)
		except:
