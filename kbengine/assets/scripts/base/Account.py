...
class Account(KBEngine.Proxy, BaseEntity):
	...
	def onEntitiesEnabled(self):
		...
		# DEBUG_MSG("Account{}::onEntitiesEnabled:accountName = {}, data = {}".format(self.id, self.__ACCOUNT_NAME__, self.getClientDatas()))
	...
	def reqCreateAvatar(self, globalBirthDict):
		...
		DEBUG_MSG('Account(%i) name[%s]::reqCreateAvatar: %d' % (self.id, self.__ACCOUNT_NAME__, props.get("userId", 0)))
