...
class iFriends:
	...
	def handleFirendByDBID(self, fromuuid, dbid, typeId):
		...
		if typeId == 0:
			sqlSentence = "INSERT INTO "+switch.DB_NAME+".tbl_Avatar_attentionYouPlayerList (parentID, sm_value) VALUES(%i, %i);"
		elif typeId == 1:
			sqlSentence = "DELETE FROM "+switch.DB_NAME+".tbl_Avatar_attentionYouPlayerList WHERE parentID=%i and sm_value=%i;"
		elif typeId == 2:
			sqlSentence = "DELETE FROM "+switch.DB_NAME+".tbl_Avatar_attentionPlayerList WHERE  parentID=%i and sm_value=%i;"

		# INSERT sm_name,sm_wealth FROM switch.DB_NAME.tbl_Avatar;
	...
	def sendFriendGiftByDBID(self, dbid, gift):
		...
		def insertDatabaseCallBack(result, num, error):
			...
			# 		KBEngine.executeRawDatabaseCommand("INSERT INTO "+switch.DB_NAME+".tbl_Avatar_giftList_attachment (parentID, sm_itemId, sm_count) VALUES(%i, %i, %i);" 
			# 			% (int(result[0][0]), item["itemId"], item["count"]), None)
			
			# KBEngine.executeRawDatabaseCommand("SELECT `id` FROM "+switch.DB_NAME+".tbl_Avatar_giftList WHERE sm_mid = %i;" % gift["mid"], queryDatabaseCallBack)

		KBEngine.executeRawDatabaseCommand("INSERT INTO "+switch.DB_NAME+".tbl_Avatar_canGetFriendGiftList (parentID, sm_gid, sm_time) VALUES(%i, %f, %f);"
