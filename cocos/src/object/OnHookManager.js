	...
	if (this.waitLeftTime != null) {
		...
		if (h1global.curUIMgr.gameRoomUIIsShow && h1global.curUIMgr.gameRoomUIIsShow()) {
			if (this.waitLeftTime <= 0) {
				h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_wait_time_left", 0);
				this.waitLeftTime = null;
			} else {
				h1global.curUIMgr.roomLayoutMgr.notifyObserver(const_val.GAME_ROOM_UI_NAME, "update_wait_time_left", this.waitLeftTime);
			}
		}
	}
};
