...
var UICommonWidget = {
    ...
    //     blinds = blinds or h1global.player().curGame.blinds;
    //     if num < blinds {
    //         num = blinds
    //     }
    ...
    create_check_box_group:function(parent, chx_name, chx_num, chx_label_name, cbk_fuc){
        var chx_list = [];
        //这是在复选框上加透明图片来增加复选框的触摸区域，如果不需要增大触摸区域，这里需要修改一下
        function chx_touchregion_event(sender, eventType) {
            if(eventType == ccui.Widget.TOUCH_ENDED) {
                for (var i = 0; i < chx_list.length; i++) {
                    if (sender.getParent() != chx_list[i]) {
                        chx_list[i].setSelected(false);
                        chx_list[i].setTouchEnabled(true);
                    }else{
                        cbk_fuc(i);
                        sender.getParent().setSelected(true);
                        sender.getParent().setTouchEnabled(false);
                    }
                }
            }
        }
        for (var i = 0; i < chx_num; i++) {
            ...
            var chx_label = undefined;
            if(chx_label_name){
                chx_label = ccui.helper.seekWidgetByName(parent, chx_label_name + String(i + 1));
            }
            chx_list.push(chx);
            this.create_touch_region(
                chx,
                cc.p(0, 0.5),
                cc.p(0, chx.getContentSize().height * 0.5),
                cc.size(chx.getContentSize().width + chx_label_name?chx_label.getString().length * chx_label.getFontSize():0, chx.getContentSize().height),
                chx_touchregion_event
            );
        }
        chx_list[0].setTouchEnabled(false);
    },
