var UIBase = cc.Layer.extend({
	...
	loadRes:function(){
		...
		if(!this.rootUINode){
            ...
            for(var snippet in this.containUISnippets){
                this.containUISnippets[snippet].loadRootUINode();
            }
	...
	unloadRes:function(){
		if(this.rootUINode && this.is_show === false){
			...
			this.rootUINode = null;
            for(var snippet in this.containUISnippets){
                this.containUISnippets[snippet].rootUINode = null;
            }
			this.hasLoadRes = false;
		}
	},
	...
	show:function(showCallback){
		...
		if(this.hasLoadRes){
			this.initUI();
            for(var snippet in this.containUISnippets){
                this.containUISnippets[snippet].initUI();
			}
			this.hasLoadRes = false;
		...
		if (is_show !== true){
			this.onShow();
            for(var snippet in this.containUISnippets){
                this.containUISnippets[snippet].onShow();
            }
		}
		if(showCallback){
			showCallback();
	...
	hide:function(){
		...
		this.onHide();
        for(var snippet in this.containUISnippets){
            this.containUISnippets[snippet].onHide();
        }
